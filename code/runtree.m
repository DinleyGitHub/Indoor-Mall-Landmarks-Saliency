...
elseif ischar(ID) && strcmpi(ID,'valbest')
    ...
    gp.fitness.returnvalues{gp.state.current_individual} = gp.results.valbest.returnvalues;   %把验证集中最好的thate传给当前代，当前代又传给了fitness funtion 
    
    treestrs = gp.results.valbest.eval_individual; %验证集中最好的个体
    ...
    fitness=feval(gp.fitness.fitfun,treestrs,gp); %用验证集中最好的个体去训练数据中再走一遍，重新计算得到了新的gp.results.valbest.returnvalues  
