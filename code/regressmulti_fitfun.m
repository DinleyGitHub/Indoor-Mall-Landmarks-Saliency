...
ypredtrain = geneOutputs * theta;  %验证集中最好的模型和参数用在了训练集，看预测结果如何
% 
% %calculate RMS prediction error (fitness)
err = gp.userdata.ytrain - ypredtrain;
fitness = sqrt(((err'*err)/numData));
%fitness = cal_rank_fitness(gp.userdata.train_rank, ypredtrain);
...
if gp.state.run_completed
    ...
    if (isfield(gp.userdata,'xval')) && (isfield(gp.userdata,'yval')) && ...
        ...
        plotValidation = 1;
        evalstr = strrep(evalstr,'.xtrain','.xval');  %把训练集换成验证集
        ...
        for i=1:numGenes
            ind = i+1;
            eval(['geneOutputsVal(:,ind)=' evalstr{i} ';']);  %用验证集计算了一把
        end
    ...
    if (isfield(gp.userdata,'xtest')) && (isfield(gp.userdata,'ytest')) && ...
        ...
        %fitnessTest = cal_rank_fitness(gp.userdata.test_rank, ypredtest);
