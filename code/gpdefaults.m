...
gp.nodes.const.about = 'Ephemeral random constants';  %临时随机向量   
gp.nodes.const.num_dec_places = 4;  
gp.nodes.const.range = [-10 10];     %ERC range        
gp.nodes.const.p_ERC = 0.1; %probability of generating an ERC when creating a leaf node创建叶节点时生成ERC的概率
gp.nodes.const.p_int = 0; %probability of generating an integer ERC 生成整数ERC的概率
