...
if strncmpi('regressmulti',func2str(gp.fitness.fitfun),12)
    ...
    for i=1:num2scan  %这么多模型，统计每个模型中各个属性出现的次数，总计一起
        model = gp.pop{i};
        hitvec = hitvec + scangenes(model,numx);  % hitvec同一位置叠加了
    end
...
if graph
    ...
    a = gca;
    bar(a,hitvec);
    if ~verLessThan('matlab','2018R') %R2014b
        a.Children.FaceColor = [0 0.45 0.74];
        a.Children.BaseLine.Visible = 'on';
    else
        b = get(a,'Children');
    ...
    %title([[gp.runcontrol.num_gen,gp.runcontrol.pop_size],titlestr],'FontWeight','bold');
