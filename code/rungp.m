function [gpout] = rungp(configFile,train_porp,train_rank,test_porp,test_rank,val_porp,val_rank)
...
gp = feval(configFile,gp,train_porp,train_rank,test_porp,test_rank,val_porp,val_rank);
...
for run=1:gp.runcontrol.runs
    ...
    gp = gpinit(gp);                         %重点  gp.results.best []   gp.fitness.returnvalues[]
    ...
    for count=1:gp.runcontrol.num_gen
        ...
        %calculate fitnesses of population members训练集
        gp = evalfitness(gp);  %产生300个 gp.fitness.values 和 gp.fitness.returnvalues(theta),
        
        %update run statistics  %看这
        gp = updatestats(gp);  %挑选出最好的fitness所对应的个体，拿到这个个体的theta权重值和模型等其他属性值
        
        %call user defined function验证集
        %开始计算验证数据集gp.results.valbest 赋值，用训练集上最好的个体去计算验证集上的数据，
        %得到最好的个体和valfitnes
        gp = gp_userfcn(gp); 
