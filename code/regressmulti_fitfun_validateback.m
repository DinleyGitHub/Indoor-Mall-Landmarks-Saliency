...
%valfitness = cal_rank_fitness(gp.userdata.val_rank, ypredval); %sqrt(mean((y-ypredval).^2));
...
if gp.state.count == 1
    ...
    gp.results.valbest = gp.results.best;    %看着 gp.results.valbest.returnvalues 第一代在此赋值
    gp.results.valbest.valfitness = valfitness;  %valfitness验证集上的fitness均方根误差!!!!!!
end
...
%看着 gp.results.valbest.returnvalues 后代在此赋值
%%从第二代开始如果当前代的验证集的fitness小于上次的值，就把valfitness和returnvalues都换成现在这一代的
